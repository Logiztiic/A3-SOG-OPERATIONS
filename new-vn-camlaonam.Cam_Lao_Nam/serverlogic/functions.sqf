...
fnc_spawnSaboteurTeam = {
    ...
    private _existing = missionNamespace getVariable ["ActiveSaboteurUnits", []];
    _existing = _existing select { alive _x };

    private _groups = _existing apply { group _x };
    _groups = _groups arrayIntersect _groups;

    if ((count _groups) >= 2) exitWith {
        diag_log "[Saboteurs] Max saboteur groups reached — aborting.";
    };

    if ((count _existing) >= 10) exitWith {
        diag_log "[Saboteurs] Max saboteur units reached — aborting.";
    };
